...
auto_setup_qc() {
    # 已有 qc 定义则跳过
    grep -q 'qc()' ~/.bashrc 2>/dev/null && return 0
    local target="${HOME}/vps_menu.sh"
    # 脚本未落盘时（管道方式运行）下载一份固定保存
    if [ ! -s "$target" ]; then
        curl -fsSL "$SCRIPT_URL" -o "$target" 2>/dev/null || return 0
        chmod +x "$target"
    fi
    cat >> ~/.bashrc <<'EOF'
...
EOF
    echo -e "${GREEN}已自动安装 qc 快捷命令：下次在终端输入 qc 即可进入本菜单${NC}"
}
